...
from loss.loss_unetWithCLS import UnetWithCLSLoss
...
def main(cfg, device, local_rank=0):
    ...
    ### MODEL INIT
    model = UnetWithCLS(classes=cfg.n_class, encoder_name=cfg.encoder, **cfg.model_cfg)
    if distributed:
    ...
    criterion = UnetWithCLSLoss(**cfg.loss_cfg["unetWithCLS"])
    ### SOLVER
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for i_batch, sample in enumerate(tbar):
            ...
            preds, cls_preds, cls_label = model.forward(imgs, masks)
            preds = F.interpolate(preds, size=(masks.size(1), masks.size(2)), mode='bilinear')
            loss, loss_item = criterion(preds, masks, cls_preds, cls_label) 
            if distributed:
                loss.backward()
                optimizer.step()
                optimizer.zero_grad()
            ...
            if i_batch % 50 == 1 and local_rank == 0:
                tbar.set_description('Train loss: %.4f; mIoU: %.4f; mF1: %.4f, batch time: %.2f' % 
                            (train_loss / (i_batch + 1), scores_train["mIoU"], cls_scores_train["mF1"], batch_time.avg))
            # break
...
if __name__ == '__main__':
    from configs.config_unetWithCLS import Config
