    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for i_batch, sample in enumerate(tbar):
            ...
            if i_batch % 50 == 1 and local_rank == 0:
                tbar.set_description('Train loss: %.4f; seg mIoU: %.4f; cls F1: %.4f; batch time: %.2f; pseudo_seg_loss: %.4f; gt_seg_loss: %.4f; cons_loss: %.4f; sim_loss: %.4f;' % 
                            (train_loss / (i_batch + 1), seg_scores_train["mIoU"], cls_scores_train['mF1'], batch_time.avg,
                            train_loss_term["pseudo_seg_loss"]/(i_batch+1), train_loss_term["gt_seg_loss"]/(i_batch+1),train_loss_term["cons_loss"]/(i_batch+1),train_loss_term["sim_loss"]/(i_batch+1)))
            # break
        seg_metrics.reset()
        cls_metrics.reset()
        batch_time.reset() 
